# Esme
A fork of Minwaita which is in turn a fork of Adwaita Compact. Unlike Minwaita, this theme is more liberal when it comes to design tweaks and spacing. The goal with this one is a clean, futuristic look with some homage to Gaia (which seems to be dead?).

## Preview
![alt text](preview-1.png "Preview 1")
![alt text](preview-2.png "Preview 2")
![alt text](preview-3.png "Preview 3")

## Original projects
- https://github.com/abihf/adwaita-compact
- https://github.com/horst3180/vertex-theme
- https://github.com/michael-rapp/gnome-shell-extension-hide-app-icon
- https://micheleg.github.io/dash-to-dock/
- https://git.gnome.org/browse/gtk+/tree/gtk/theme/Adwaita/
- https://git.gnome.org/browse/gnome-themes-standard/tree/themes/Adwaita
- https://git.gnome.org/browse/gnome-shell-sass/
